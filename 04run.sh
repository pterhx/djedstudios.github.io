...
EB_CONFIG_DOCKER_PORT=`cat $EB_CONFIG_APP_CURRENT/Dockerfile | grep ^EXPOSE | awk '{print $2}'`
...
docker run -d \
	...
	-e SENTRY_RAVEN_DSN=$SENTRY_RAVEN_DSN \
	-e USER_MEDIA_STORAGE_BUCKET=$USER_MEDIA_STORAGE_BUCKET \
		   "${EB_CONFIG_DOCKER_ENV_ARGS[@]}" \
		   "${EB_CONFIG_DOCKER_VOLUME_MOUNTS[@]}" \
