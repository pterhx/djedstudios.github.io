...
# tag sample app image under aws_beanstalk namespace
FROM_IMAGE=`cat Dockerfile | grep ^FROM | head -n 1 | awk '{ print $2 }'`
if [ "`echo $FROM_IMAGE | awk -F ':' '{ print $1 }'`" = "$EB_CONFIG_SAMPLE_APP_IMAGE" ] && ! docker images -a | awk '{ print $1 ":" $2 }' | grep -q "$FROM_IMAGE"; then
	# build an "aws_beanstalk/" image for our sample app

	ORIGINAL_FROM_IMAGE=`echo $FROM_IMAGE | awk -F '/' '{ print $2 }'`
	REGION=`curl http://169.254.169.254/latest/dynamic/instance-identity/document | jq -r .region`

	mkdir -p /tmp/eb-docker

	cat > /tmp/eb-docker/Dockerfile <<EOF
FROM	$ORIGINAL_FROM_IMAGE
RUN		sed -i -e 's/archive.ubuntu.com/${REGION}.ec2.archive.ubuntu.com/g' /etc/apt/sources.list
EOF

	docker build -t $FROM_IMAGE /tmp/eb-docker 2>&1 | tee /tmp/docker_build.log

	DOCKER_BUILD_EXIT_CODE=${PIPESTATUS[0]}
	if [ $DOCKER_BUILD_EXIT_CODE -ne 0 ]; then
			LOG_TAIL=`cat /tmp/docker_build.log | tail -c 200`
			error_exit "Failed to build Docker image $FROM_IMAGE: $LOG_TAIL. Check snapshot logs for details." $DOCKER_BUILD_EXIT_CODE
	fi
...
# copy env variables
/opt/elasticbeanstalk/containerfiles/support/generate_env.py >> environment_vars
